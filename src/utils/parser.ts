...
const ANDROID_REGEX = /^(\d{1,4}[-/.]\d{1,4}[-/.]\d{2,4}),?\s+(\d{1,2}:\d{2}(?::\d{2})?(?:\s*[APap][Mm])?)\s*-\s*(.*)$/;

// 2. iOS style: "[dd/mm/yyyy, hh:mm:ss] Sender: Message"
const IOS_REGEX = /^\[(\d{1,4}[-/.]\d{1,4}[-/.]\d{2,4}),?\s+(\d{1,2}:\d{2}(?::\d{2})?(?:\s*[APap][Mm])?)\]\s*(.*)$/;

// 3. Simple Fallback style (some exports do not have '-' or brackets): "dd/mm/yyyy, hh:mm: Sender: Message"
const FALLBACK_REGEX = /^(\d{1,4}[-/.]\d{1,4}[-/.]\d{2,4}),?\s+(\d{1,2}:\d{2}(?::\d{2})?(?:\s*[APap][Mm])?):\s*(.*)$/;
 ...
 */
function parseDate(dateStr: string, timeStr: string, isDDMM: boolean): Date | null {
  try {
