...
export default function ParticipantSelector({
  ...
  const totalMessages = Object.values(senderCounts).reduce((acc, count) => acc + count, 0);

  return (
    <div id="participant-selector-container" className="w-full max-w-xl mx-auto px-4 py-8 flex flex-col items-center">
      <div className="w-full bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-neutral-100 mb-6">
        <div className="flex items-center gap-2 mb-4 text-emerald-600">
          <BookOpen className="w-5 h-5" />
          <span className="font-sans font-semibold text-xs uppercase tracking-wider">File Loaded Successfully</span>
        </div>

        <h2 className="font-display text-2xl font-semibold text-neutral-900 mb-2 truncate" title={fileName}>
          {fileName}
        </h2>
        <p className="text-neutral-500 font-sans text-xs md:text-sm mb-6 flex items-center gap-2">
          <MessageSquare className="w-4 h-4 text-neutral-400" />
          Parsed {totalMessages.toLocaleString()} messages across {participants.length} participants.
        </p>
        ...
        <div className="border-t border-neutral-100 pt-6 mb-6">
          ...
          <p className="text-neutral-400 font-sans text-xs mb-4">
            Select your participant name to align your messages on the right (as outgoing). All other participants will be shown on the left (as incoming).
          </p>
